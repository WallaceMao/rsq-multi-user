 ...
 */
class TeamMainUserApiSpec extends BaseTeamApiGebSpec {
...
//                suiteEnv.team8ForCreate
    ...
    def "user1 is not team user and main user is user1"(){
        ...
        println "----id----${userId}"
        resp = loginAndSetMainUser(suiteEnv.teamUser1, [userId: userId])

        then:
        checkSetMainUser(resp, [userId: userId])
    }
    ...
    def "user2 create team2 -> main user can only set to user2"(){
        ...
        when:
        resp = loginAndCheck(suiteEnv.teamUser2)
        long userId = suiteEnv.teamUser2.id
        println "----id----${userId}"
        resp = loginAndSetMainUser(suiteEnv.teamUser2, [userId: userId])
